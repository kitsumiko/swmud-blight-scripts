...
State.prompt = {
  ...
  level_regexp = regex.new("([a-zA-Z]+(?: [a-zA-Z]+)*)\\s*:\\s*([0-9]+)"),
  ...
  prompt_re = regex.new("^([^ ]*)/([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*)/([^ ]*) ([^ ]*) ([^ ]*)/([^ ]*) ([^ ]* ?[^ ]*? ?[^ ]*?) >"),
  droid_match = regex.new("^\\s*(C3|T5|SLR|GNK|B1|RX|G2|MLR|R4P|NR|FX|IF|IG|RA|XLR|BLX|T7|C5|S9E|B2|DD|ALR|MSE|OOM|R8|2\\-1C|HK|FA\\-5|X7)(.*) ((\\(Yours\\)|\\[Listening\\]))$"),
