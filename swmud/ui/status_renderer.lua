...
function StatusRenderer.render()
  ...
  local success, err = pcall(function()
    ...
    if update_skill_status then
      update_skill_status()
    end
    if update_target_status then
      update_target_status()
    end
    if update_durable_skill_status then
    ...
    vitals_line = vitals_line .. (C_RESET or "")
    ...
    if move_cmd == "" then
      move_cmd = (C_BYELLOW or "") .. "<set_move>" .. (C_RESET or "")
    end
    local move_line = (C_RESET or "") .. "Move: " .. (C_BYELLOW or "") .. tostring(move_cmd) .. (C_RESET or "")
    if PROMPT_INFO.move_cmd ~= "" then
      move_line = move_line .. (C_BYELLOW or "") .." (set_move)" .. (C_RESET or "")
    end
    ...
    if autos_diff > 10*60 and mud.is_connected() then
      blight.output((C_BYELLOW or "") .. "Autosave Missed! " .. os.date("%c") .. (C_RESET or ""))
      mud.send("", {gag=1,})
  ...
  if not success then
    blight.output("Error in status_draw: " .. tostring(err))
  end
end
