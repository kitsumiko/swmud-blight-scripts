...
create_skill("decay", "^The (.+ decays) under your influence\\.", "^You (fail to decay) the ", 2, nil)
create_skill("improve", "^You (successfully improve) the ", "^You (fail to improve) the ", 3, nil)
create_skill("improve", nil, "^You (attempt to improve the lightsaber) handle but end up damaging it!", 3, nil)
create_skill("construct", "^You (take the holographic projector) ", "^You (fail to get the circuitry working) ", 150, nil)
create_skill("construct", "^You (change the circuitry) ", "^You fail to construct a .*\\.", 150, nil)
create_skill("construct", "^You (spend some time tinkering) ", nil, 150, nil)
create_skill("construct", "^You (take the charging mechanism) ", nil, 150, nil)
create_skill("construct", "^You (alter the .+valves) ", nil, 150, nil)
create_skill("construct", "^You (remove the spikes from the knuckles) ", nil, 150, nil)
...
create_skill("lightening", "^Lightning (arcs from your fingers) ", "^You call forth the lightning, but (it fails to come.)", 6, nil)
create_skill("enhatt", "^Through the power of the Force (you enhance your .+) ability\\.", "^The Force (denies you the ability to enhance) that ability\\.", 6, nil)
create_skill("mwipe", "^You (concentrate your Sith powers) on .+, causing ", "^You concentrate your Sith powers on .+ (but lose your concentration)\\.", 6, nil)
create_skill("jpack", "^You (bring the jetpack) to a stop\\.", "^You (fail to operate) the jetpack correctly\\.", 2, nil)
create_skill("picture", "^(Map of your location):", "^You (fail to picture) your surroundings\\.", 2, nil)
create_skill("bioeng", "^The medbots finish their procedure.+\\.  (The procedure was a success)!", "^The medbots finish their procedure.+\\.  (The procedure has failed)\\.", 0, nil)
create_skill("bioeng", nil, "^You are in a great deal of pain, and judging by the medbot's readouts, (the procedure was a failure)\\.", 0, nil)
create_skill("syringe", "^You (stick a needle) deep into your own flesh\\.", "^You try to inject yourself, (but wimp out) and get .+ everywhere\\.", 2, nil)
create_skill("syringe", "^The (needle buries itself) deep into the flesh of .+!", "^You try, but (fail to inject)\\.", 2, nil)
create_skill("fpush", "^You (use the Force to push) everyone", "^You (lose your concentration) and fail\\.")
create_skill("farsight", "^You (see into .+ environment) and you see:", "^You (fail to see) .+", 2, nil)
create_skill("fshield", "^You (erect a shield) of Force around yourself\\.", "^You (fail to erect) a shield of Force", 6, nil)
create_skill("bmed", "^You feel your (control over the Force increase)\\.", "^Your (concentration is broken) and you can no ", 150, nil)
create_skill("superheal", "^You heal yourself completely\\.", "Nothing happens\\.", 600, nil, 1)
create_skill("she", nil, nil, 3, nil, 1)
create_skill("cfear", "^You create the (sound of the Krayt Dragon)\\.", "^You (make a small, pathetic squeak) quite ")
create_skill("project", "^As (you project your image), you see the surroundings of your image...\\.", "^You (attempt to project) your image to .+, but you lose", 3, nil)
create_skill("disarm", "^You (succeed in disarming) your opponent!", "^You (fail to disarm) your opponent\\.", 2, nil)
create_skill("droid_construct", "^You (slowly construct) the shell of the droid from the scrap\\.", "^You (try to construct a droid), but fail\\.", 450, nil)
create_skill("droid_construct", nil, "^You fail to construct an .* Model\\.", 450, nil)
...
create_skill("overhaul", "^(You spend a good hour of your day fixing every problem) you can find with .*\\. When you are done, it gleams as if new\\.", "^You spend an hour of your day fiddling with (.*) but it", 3, nil)
create_skill("dmodify", "^You have (successfully installed) .*", "^You (try to modify) the droid, but fail", 150, nil)
...
create_skill("sneak", "^You (quietly sneak) into the next room\\.", "^(Everybody notices you as you attempt to sneak) out of the room\\.", 4, nil, nil, "You don\\'t feel like being sneaky and instead just walk\\.")
