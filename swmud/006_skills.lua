...
create_skill("absorb", "^You (concentrate on absorbing) energy directed at you\\." , "^You (fail to achieve) the proper concentration required\\.", 300, 6)
-- create_skill("pres", "^You (remove the drugs|remove some of the drugs|purge your body) ", "^Nothing happens\\.", 1)
create_skill("scloak", "^You (draw the Dark Side) around you to hide your presence\\.", "^You (lose control) of the Force and fail\\.", 5, 5, 1)
...
create_skill("decay", "^The (.+ decays) under your influence\\.", "^You (fail to decay) the ", 5, 5)
create_skill("improve", "^You (successfully improve) the ", "^You (fail to improve) the ", 6, 6)
create_skill("improve", nil, "^You (attempt to improve the lightsaber) handle but end up damaging it!", 6, 6)
create_skill("construct", "^You (take the holographic projector) ", "^You (fail to get the circuitry working) ", 300, 6)
create_skill("construct", "^You (change the circuitry) ", "^You fail to construct a .*\\.", 300, 6)
create_skill("construct", "^You (spend some time tinkering) ", nil, 300, 6)
create_skill("construct", "^You (take the charging mechanism) ", nil, 300, 6)
create_skill("construct", "^You (alter the .+valves) ", nil, 300, 6)
create_skill("construct", "^You (remove the spikes from the knuckles) ", nil, 300, 6)
...
create_skill("bmed", "^You feel your (control over the Force increase)\\.", "^Your (concentration is broken) and you can no ", 300, 6)
...
create_skill("project", "^As (you project your image), you see the surroundings of your image...\\.", "^You (attempt to project) your image to .+, but you lose", 6, 6)
create_skill("disarm", "^You (succeed in disarming) your opponent!", "^You (fail to disarm) your opponent\\.", 2, 2)
create_skill("droid_construct", "^You (slowly construct) the shell of the droid from the scrap\\.", "^You (try to construct a droid), but fail\\.", 900, 8)
create_skill("droid_construct", nil, "^You fail to construct an .* Model\\.", 900, 8)
...
create_skill("overhaul", "^(You spend a good hour of your day fixing every problem) you can find with .*\\. When you are done, it gleams as if new\\.", "^You spend an hour of your day fiddling with (.*) but it", 6, 6)
create_skill("dmodify", "^You have (successfully installed) .*", "^You (try to modify) the droid, but fail", 300, 9)
create_skill("dmodify", nil, "^You try to modify .*, but fail", 300, 9)
create_skill("cureall", "You (perform cureall surgery)", "^You try, but (fail to perform cureall surgery)", 300, 4)
create_skill("ionshield", "(You shield the circuitry)", "^(You fail to shield the circuitry)", 6, 6)
...
create_skill("linehack", "^You crack into the line\\.", "^You fail to hack the line with the grace of a rancor\\.", 40, 4)
...
create_skill("hack_bank", "^You have managed to funnel.*", nil, 43200, 4)
